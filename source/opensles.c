    ...
    if (!p->cur || p->cur_pos >= p->cur_size) {
      ...
      if (!have) {
        ...
        for (int k = 0; i < frames; i++, k++) {
          ...
          acc[i * 2 + 1] += p->last_r * den / FADE_LEN;
        }
        p->last_l = p->last_r = 0; // decayed out; stay silent until resume
        return;
      }
      p->cur = b.data;
      p->cur_size = b.size;
...
static void SDLCALL audio_callback(void *ud, Uint8 *stream, int len) {
  ...
  // The engine's buffer-queue callback runs on THIS SDL audio thread and reads
  // its stack canary from tpidr_el0+0x28; SDL created the thread (not our
  // pthread shim), so set up its TLS guard or the first guarded call faults.
  tls_setup_guard();
...
// No-op fallback interface for OpenSL features we don't implement
// (PlaybackRate/EffectSend). Returning UNSUPPORTED would fail player creation
// and the engine would then deref a NULL player; a SUCCESS-stub vtable is safe.
static SLresult stub_ok(void *self) { (void)self; return SL_RESULT_SUCCESS; }
static void *g_stub_vtable[32];
static void *g_stub_itf = g_stub_vtable; // points at g_stub_vtable[0]
static void stub_vtable_init(void) {
  for (int i = 0; i < 32; i++)
    g_stub_vtable[i] = (void *)stub_ok;
}

static SLresult mix_GetInterface(void *self, const SLInterfaceID iid, void *pInterface) {
  (void)self; (void)iid;
  if (pInterface) *(void **)pInterface = &g_stub_itf;
  return SL_RESULT_SUCCESS;
}
...
static SLresult player_GetInterface(void *self, const SLInterfaceID iid, void *pInterface) {
  ...
  if (iid == SL_IID_PLAY) {
    ...
    *(void **)pInterface = &p->vol_vt;
    ...
    // PlaybackRate / EffectSend / etc. -> generic no-op interface so player
    // creation succeeds and method calls don't deref a NULL interface.
    // (not logged: the engine creates many SE players, flooding the log)
    *(void **)pInterface = &g_stub_itf;
  }
  return SL_RESULT_SUCCESS;
}
...
static SLresult eng_CreateAudioPlayer(void *self, SLObjectItf *pPlayer, SLDataSource *src, SLDataSink *snk,
  ...
  p->channels = 2;
  p->rate = 44100;
  p->fade = FADE_LEN; // start at full volume (fade only applies after underrun)
